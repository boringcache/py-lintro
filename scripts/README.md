...
| `verify_built_binary.sh`              | Verify a built binary responds to `--version`, the syntax-highlighting self-check and `--help`              | `./scripts/build/verify_built_binary.sh dist/nuitka/lintro`              |
| `nuitka_bytecode_plugin.py`           | Nuitka user plugin shipping pygments as bytecode instead of compiled C (#2484)                              | passed to Nuitka via `--user-plugin` by the two build scripts            |
...
| `resolve-nuitka-version.py`                 | Resolve the Nuitka version `uv.lock` pins, for the binary build cache key (#2484)                                                | `python3 scripts/ci/resolve-nuitka-version.py`                                                                             |
